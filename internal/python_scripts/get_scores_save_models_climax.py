...
save_dir_parent="pretrained_models_small"
...
model_filter=["ClimaX", "climax"]
experiment_filter=["superemulator", "super_emulator", "single_emulator"]
climate_model_filter=["NorESM2-LM"]
# read in csv
...
for i,row in df.iterrows():
    ...
    want_experiment=True
    # filter by model and experiment type
    if (parent_folder_dir in experiment_filter):
        print("first if")
        if (model_name in model_filter):
            if parent_folder_dir=="single_emulator":
                for cm in climate_model_filter:
                    print(cm)
                    print(cm not in experiment_name)
                    if cm not in experiment_name:
                        print(f"Unwanted experiment: {(parent_folder_dir, model_name)}  {experiment_name}")
                        want_experiment=False
    ...
    if want_experiment:
        print(f"Copying {parent_folder_dir} {model_name} {experiment_name}")

...
def get_normalizer(normalizer="z", *args, **kwargs) -> NormalizationMethod:
    ...
    assert (
        normalizer in supported_normalizers
    ), f"Unsupported Normalization {normalizer} not in {str(supported_normalizers)}"
    if normalizer == "z":
...
class Normalizer:
    ...
    def get_input_normalizer(
        self, data_type: str
    ) -> Union[NP_ARRAY_MAPPING, NormalizationMethod]:
        return self._input_normalizer[data_type]

    def get_input_normalizers(
        self,
    ) -> Dict[str, Union[NP_ARRAY_MAPPING, NormalizationMethod]]:
        return {
    ...
    def set_normalizer(
        ...
        if data_type in constants.INPUT_TYPES:
            ...
            print(
                f"INFO: Setting output normalizer, after calling set_normalizer with data_type={data_type}"
            )

...
class Causalpaca_HdF5_Dataset(
...
):  # TODO: where to do the training vs testing discrimination?
    ...
    def __init__(
        ...
        if not (have_forcing):
            print(
                "WARNING: No forcing variables were given, please complete your specifications."
            )
            exit(0)
        if not (have_model):
            print(
        ...
        if self.load_h5_into_mem:
            if target == "emulator":
                ...
                dset_class = Causalpaca_HdF5_FastSingleDataset_Causal
            else:
                print(f"WARNING: Uknown target {self.target} for dataset class.")
                raise ValueError
            ...
            for exp in self.experiments:
                ...
                for model in self.models:
                    for member in self.ensemble_members:
                        ...
                        if n_samples == 0:
                            print(
                                f"wARNING: No data available for pairing: model {model} member {member} exp {exp}. Skipping."
                            )
                            continue
    ...
    def __getitem__(self, item=None) -> (Tuple[Dict[str, Tensor], Dict[str, Tensor]]):
        ...
        if self.target == "emulator":
            ...
            assert (
                n_f == n_m
            ), "WARNING: Not the same time series lenght for forcing and model data."

            d_x_f = spatial_dim_f["d_x"]
            d_x_m = spatial_dim_m["d_x"]
            assert (
                d_x_f == d_x_m
            ), "WARNING: Spatial dimension of forcing and model data do not match."
            ...
            if self.tau is not None:
                ...
                assert self.tau < (
                    n_m - t1
                ), f"WARNING: Time window size {self.tau} bigger than available timeseries lenght {n_m}."
    ...
    def __init__(
        ...
        if aspect == "forcing":
            for v in variables:
                ...
                paths = [
                    (
                        f"{self.data_dir}/input4mips/{self.experiment}/{v}/{self.nom_res}/{self.freq}/{y}/"
                    )
                    for y in self.years
                ]
                ...
                full_file_names = [
                    p + f for p in paths for f in file_names if os.path.exists(p + f)
                ]
                # print(full_file_names)
                # full_file_names=[p+os.listdir(p)[0] for p in paths if os.path.exists(p)]
                if len(full_file_names) == 0:
                    print(
                        f"WARNING: No data available for {aspect} data combination: {self.experiment} for variable {v}. Skipping all variables of this combination."
                    )
                    raise ValueError
            ...
            for v in variables:
                paths = [
                    (
                        f"{self.data_dir}/CMIP6/{self.model}/{self.member}/{self.experiment}/{v}/{self.nom_res}/{self.freq}/{y}/"
                    )
                    for y in self.years
                ]
                file_names = [os.listdir(p)[0] for p in paths if os.path.exists(p)]
                ...
                if len(full_file_names) == 0:
                    print(
                        f"WARNING: No data available for {aspect} data combination: {self.model} {self.member} {self.experiment} for variable {v}. Skipping all variables of this combination."
                    )
                    raise ValueError
                self.file_names[v] = file_names
                ns.append(len(full_file_names))
            ...
            print(
                f"WARNING: Dataset class initialized with unknown aspect {aspect}. Please initialize with aspect being either 'forcing' or 'model'"
            )
            exit(0)
        ...
        for v in self.all_files:
            array_per_var.append(
                np.stack([h5py.File(f, "r")[v] for f in self.all_files[v]], axis=0)
            )
...
class Causalpaca_HdF5_SingleDataset_Causal(Causalpaca_HdF5_SingleDataset_Emulator):
    def __init__(self, *args, **kwargs):
        """
        Single Dataset for Causal pipelne.
        """
    ...
    def __getitem__(self, item=None) -> Dict[str, Tensor]:
        ...
        for v in self.all_files:
            ...
            array_per_var.append(
                np.concatenate(
                    [h5py.File(f, "r")[v] for f in self.all_files[v]], axis=0
                )
            )
...
class Causalpaca_HdF5_FastSingleDataset_Emulator(
    Causalpaca_HdF5_SingleDataset_Emulator
):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        ending = f"_{self.target}_{self.aspect}.npz"
        # get the name of the processed dataset
        processed_fname = get_processed_fname(
            data_dir=self.data_dir, name=self.name, **pkwargs, ending=ending
        )
    ...
    def _write_data(self, fname: str, data: Dict[str, np.ndarray]):
        ...
        if isinstance(data, dict) or isinstance(data, np.ndarray):
            try:
                np.savez_compressed(fname, **data) if isinstance(
                    data, dict
                ) else np.savez_compressed(fname, data)
            except OverflowError as e:
                print(
                    f"WARNING: OverflowError -> {fname} will been stored as a pickle.\n{e}"
                )
                with open(fname, "wb") as fp:
                    pickle.dump(data, fp, pickle.HIGHEST_PROTOCOL)
            except PermissionError as e:
                print(
                    f"WARNING: Tried to cache data to {fname} but got error {e}. "
    ...
    def copy_to_slurm_tmp_dir(
        self, aspect
    ):  # TODO: figure out if we want to inherit that or pass it
        pass
    ...
    def __getitem__(self, item=None) -> Dict[str, Tensor]:
        ...
        if (
            isinstance(self.data["Data"], np.ndarray)
            and len(self.data["Data"].shape) == 0
        ):
...
class Causalpaca_HdF5_FastSingleDataset_Causal(
    Causalpaca_HdF5_FastSingleDataset_Emulator
):
    def __init__(self, *args, **kwargs):
        """ Fast Single Dataset for Causal pipeline """
        super().__init__(*args, **kwargs)
    ...
    def __getitem__(self, item) -> Dict[str, Tensor]:
        ...
        @returns:
            data (Dict[str, np.ndarray]): Ditionary storing the sliced time window data under the key "X" and the target data point of the respective time window under key "Y". If no tau is given, the full time series instead of a window is returned.
        ...
        if isinstance(self.data["Data"], np.ndarray):
            data = torch.from_numpy(self.data["Data"]).float()
        else:
            # TODO: not sure what the else is here
            raise TypeError

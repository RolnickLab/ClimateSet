...
class DataLoader:
    def __init__(
        ...
        if consider_forcing:
            (
                self.forcing_data,
                self.forcing_exp_to_index,
                avail_years,
            ) = self._load_data_forcing()
        ...
        if len(self.model_vars) >= 1:
            for m in self.ensemble_members:
                member_data, member_exp_to_index = self._load_data_model_ensemble(
                    m, all_avail_years
                )
                ...
                if consider_forcing:
                    ...
                    ), "WARNING: Spatial dimensions of forcing and member data does not match!"
                else:
                    self.d_x = member_data.shape[-1]
                ...
                if load_mix:
                    if consider_forcing:
                        for e in self.experiments:
                            if e in (
                                self.forcing_exp_to_index.keys()
                                & member_exp_to_index.keys()
                            ):
                                ...
                                self.data.append(
                                    np.stack(
                                        [
                                            self.forcing_data[f_start:f_end],
                                            member_data[m_start:m_end],
                                        ],
                                        axis=2,
                                    )
                                )
    ...
    def _load_data(self, member="r1i1p1f1"):
            ...
            for y in self.years:
                ...
                if d_m >= 1:
                    ...
                    for i, v in enumerate(self.model_vars):
                        ...
                        if not isExist:
                            print(
                                f"WARNING: Experiment, variable, year pair does not exist: {e,v,y}. Skipping."
                            )
                            enough_data = False
                            continue
                        ...
                        if len(f_list) == 0:
                            print(
                                f"WARNING: No data available for the pair {e, v}. Skipping."
                            )
                            enough_data = False
                            continue
                        f_name = f_list[0]
                ...
                if enough_data:
                    ...
                    print(
                        f"WARNING: Not all vars available for the specified year: {y}. Skipping."
                    )
                    continue
        ...
        if len(data_per_exp) == 0:
            print(
                "WARNING: No data available. Please check your specifications and make sure you have all variables for all years specfiied."
            )
            raise ValueError
    ...
    def _load_data_model_ensemble(self, member, years):
        ...
        if len(data_per_exp) == 0:
            print(
                f"WARNING: No data available for member {member}. Please check your specifications and make sure you have all variables for all years specfiied."
            )
            return None, None
        data = np.concatenate(data_per_exp)
    ...
    def sample_mix(self, batch_size=None, valid=bool):
        ...
        assert (
            self.load_mix
        ), "You want to sample mixed data but due to storage efficiency it is not yet loaded. Please initialize the data_loader with load_mix=True to load mixed data."
    ...
    def sample(self, batch_size: int, valid: bool = False):
        ...
        for i, exp in enumerate(experiments):
            ...
            if len(self.forcing_vars) >= 1:
                ...
                try:
                    ...
                    print(
                        f"WARNING: Experiment {exp} not existent in forcing {aspect} dataset."
                    )
                    continue
                ...
                if not (forcing_data_per_batch.shape[-1] != self.d_x):
                    print(
                        f"WARNING: Spatial resolution for forcing data do not match, got {forcing_data_per_batch.shape[-1]} but should be {self.d_x}. Skiping."
                    )
                    continue

                # one big time series = sample along time dimension
                if forcing_n == 1:
                    random_idx_forcing = np.random.choice(
                        forcing_data_per_batch.shape[1], replace=True, size=batch_size
                    )

                    for i, jdx in enumerate(zip(random_idx_forcing)):
                        x_forcing = np.squeez(forcing_data_per_batch)[
                            jdx - self.tau : jdx + t1, :, :
                        ]
                ...
                #  multiple time series (e.g. several years), sample over years
                    ...
                    random_idx_forcing = np.random.choice(
                        forcing_data_per_batch.shape[0], replace=True, size=batch_size
                    )
            ...
            if len(self.model_vars) >= 1:
                ...
                for m in self.ensemble_members:
                    ...
                    try:
                        ...
                        aspect = "validation" if valid else "training"
                        print(
                            f"WARNING: Experiment {exp} member r{m} pair not existent in {aspect} dataset."
                        )
                        continue
                    ...
                    if model_data_per_batch.shape[-1] != self.d_x:
                        print(
                            f"WARNING: Spatial resolution for model data {exp} {m} do not match, got {model_data_per_batch.shape[-1]} but should be {self.d_x}. Skiping."
                        )
                        continue

                    # one big time series = sample along time dimension
                    if model_n == 1:
                        random_idx_model = np.random.choice(
                            model_data_per_batch.shape[1], replace=True, size=batch_size
                        )
                        for i, jdx in enumerate(zip(random_idx_model)):
                            x_model = np.squeez(model_data_per_batch)[
                                jdx - self.tau : jdx + t1, :, :
                            ]
                    ...
                    #  multiple time series (e.g. several years), sample over years
                        ...
                        random_idx_model = np.random.choice(
                            model_data_per_batch.shape[0], replace=True, size=batch_size
                        )
                    ...
                    if have_model_data & have_forcing_data:
                        if not (model_n == forcing_n):
                            print(
                                f"WARNING: Not the same number of samples experiment {exp} and member {m} for forcings and model variables. Skipping "
                            )
                            continue
                        ...
                        print(
                            f"WARNING: No data for experiment {exp} and member {m} for forcings and model variables. Skipping."
                        )
                        continue
        ...
        if (len(full_x) == 0) or (len(full_y) == 0):
            print(
                f"WARNING: Apperantly no data available that fits the specifications. Please check your dataset and specifications."
            )
            exit(0)
        ...
        for i, exp in enumerate(experiments):
            try:
                ...
                if ((i + 1) == len(experiments)) & (len(data_per_batch) == 0):
                    print(
                        "WARNING: No data found, please confirm your experiment selection. Aborting."
                    )
                    raise exit(0)
                continue
            ...
            if n == 1:
                random_idx = np.random.choice(
                    dataset_idx, replace=True, size=batch_size
                )
                for i, idx in enumerate(random_idx):
                    # first dimension is 1
                    x[i, :] = np.squeeze(self.data_per_batch)[
                        idx - self.tau : idx + t1, :, :
                    ]

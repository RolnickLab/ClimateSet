...
class LinearMasked(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.mean(torch.sum(0.5 * ((y - y_hat))**2, dim=0))
...
def train(adj, data, idx_train, idx_valid, max_iter, batch_size):
    ...
    for iter in range(max_iter):
        ...
        if iter % 100 == 0:
            print("Iteration: {}, score_train: {:.5f} , score_valid : {:.5f}, best_train_score : {:.5f}. best_valid_score {:.5f}, \
                  patience: {}".format(iter, train_score, valid_score, best_train_score, best_valid_score, patience))
        if patience == 0:
            flag_max_iter = False
            break
